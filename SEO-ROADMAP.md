...
Folder-style pages (see cycle 15 for the region structure), generated by `_build/build.pl` (see `_build/README.md`; edit the sources in `_build/content/`, never the generated HTML). Header: Köln | Karlsruhe & Mittelbaden | A5 | Ratgeber | Kontakt. Footer: 5 columns. Visible breadcrumbs + BreadcrumbList schema on every page except the home page.
...
### Cycle 15 (2026-09-26) – restructure by region, Köln districts, motorways, guides
- **Structure changed.** Pages are now clean folder URLs (`folder/index.html`), grouped by region: `/koeln/…`, `/karlsruhe/…`, shared `/ratgeber/…`; `kontakt`, `impressum`, `datenschutz`, `danke` at the root. The root `index.html` is a neutral home with two region cards (Köln & Umgebung, Karlsruhe & Mittelbaden). The old Köln home is now `/koeln/`. Old flat `.html` URLs are removed (site was never indexed, so no redirects needed). The mapping lives in `%PATH` in `_build/build.pl`; menus per region in `%MENU`.
- Internal links stay relative (GitHub Pages project path). After the custom domain (B2) they still work; only `$BASE` needs to change.
- New Köln pages: 9 district pages (`/koeln/innenstadt/`, `ehrenfeld`, `nippes`, `lindenthal`, `rodenkirchen`, `porz`, `kalk`, `muelheim`, `chorweiler`), 4 motorway pages (`a57`, `a59`, `a555`, `leverkusener-bruecke`), hub `/koeln/stadtteile/` links all 9. Each district page links two neighbours, the Köln overview and one guide.
- New guides: Panne im Tunnel, Umweltzone Köln, Abschleppen in Köln: Kosten (no prices).
